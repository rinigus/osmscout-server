...
Dialog {
    ...
    SilicaFlickable {
        ...
        Column {
            ...
            Label {
                text: qsTr("Geocoder is responsible for responding to search requests. For that, it parses the search string and finds the corresponoding objects on a map.")
            ...
            ElementSwitch {
                ...
                secondaryLabel: qsTr("When selected, a libpostal-based geocoder will be used to resolve all search requests. Note that it requires additional databases for language and user input parsing and geocoding. The additional requirements could be significant, depending on selected languages and country")
            }
            ...
            ElementSwitch {
                ...
                secondaryLabel: qsTr("In addition to libpostal, primitive parser allows you to specify administrative hierarchy of a searched object by separating components with comma. Sometimes, when libpostal fails to parse the request correctly, this parser allows you to overcome the issue. Example: house_number, street, town.")
            }
            ...
            ElementSwitch {
                ...
                secondaryLabel: qsTr("When selected, libpostal databases will be loaded to the memory only while parsing the request. As a result, while search would take longer time, the memory overhead of libpostal is small in idle.")
            }
        }
