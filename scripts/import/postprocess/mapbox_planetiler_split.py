...
parser.add_argument(
    "--split-level", type=int, default=7, help="Zoom level at which to split"
)
...
r = cur.execute(
    "select min(zoom_level) as minz, max(zoom_level) as maxz from tiles_shallow"
).fetchone()
...
for tri, v in tofile.items():
    ...
    if intersects(polys, b):
        ...
        if os.path.exists(f):
            print(
                f"Export file {f} exists already. Please clear output directory to avoid clashes"
            )
            sys.exit(1)
...
for k, tiles in filtered.items():
    ...
    with sqlite3.connect(f) as conn_out:
        ...
        cur_out.execute(
            "INSERT INTO metadata(name, value) VALUES(?, ?)", ("minzoom", min_z)
        )
        cur_out.execute(
            "INSERT INTO metadata(name, value) VALUES(?, ?)", ("maxzoom", max_z)
        )
        for r in cur.execute("SELECT name, value FROM metadata"):
            if r[0] not in ["bounds", "center", "minzoom", "maxzoom"]:
                cur_out.execute(
                    "INSERT INTO metadata(name, value) VALUES(?, ?)", (r[0], r[1])
                )
        ...
        for i in data_ids:
            r = cur.execute(
                "SELECT tile_data_id, tile_data FROM tiles_data WHERE tile_data_id=:id",
                dict(id=int(i)),
            ).fetchone()
            cur_out.execute(
