...
//    path = "http://localhost:8080/tile/1/2/3
//    sep = "/tile/"
...
void RequestMapper::service(HttpRequest& request, HttpResponse& response)
{
    ...
    QStringList command;
    if (contains(path, "/tile/", command)) // Tile
    {
        ...
        int shift = command[0].toInt();
        int scale = command[1].toInt();
        int z = command[2].toInt();
        int x = command[3].toInt();
        int y = command[4].toInt();

        int ntiles = 1 << shift;

        qDebug() << ntiles << " " << scale << " " << z << " " << x << " " << y;

        QByteArray bytes;
        if ( !osmScoutMaster->renderMap(96*scale/ntiles, z + shift, 256*scale, 256*scale,
